...
import { isOrgAreaRole, isProviderAreaRole, isSuperAdminRole, normalizeUserRole } from "./roles";
...
describe("role helpers", () => {
  ...
  it("normalizes backend role values for frontend guards", () => {
    expect(normalizeUserRole("user")).toBe("USER");
    expect(normalizeUserRole("provider")).toBe("PROVIDER");
