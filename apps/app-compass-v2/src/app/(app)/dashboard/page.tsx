  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 pb-12">
      ...
      <div className="flex items-end justify-between gap-4">
        ...
        <Link
          href="/readiness"
          className="hidden sm:inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-brand-50 text-brand-700 text-body-sm font-heading font-black hover:bg-brand-100 transition-all shadow-sm border border-brand-200/50"
        >
      ...
      <div className="grid gap-6 lg:grid-cols-[1.4fr,1fr,1fr]">
        <div className="card-surface p-6 border-2 border-brand-200/40 bg-gradient-to-br from-white via-brand-50/20 to-silver-50/50 shadow-glass-sm rounded-[2rem]">
          ...
          <div className="mt-4 flex items-center gap-2">
            ...
            <p className="text-[10px] font-black text-brand-600 uppercase tracking-wider whitespace-nowrap">{lifecycle.progressLabel}</p>
          </div>
          <Link href={lifecycle.href} className="mt-6 inline-flex items-center gap-2 text-body-sm font-black text-brand-600 hover:text-brand-950 transition-all uppercase tracking-tight group">
            {lifecycle.cta}
        ...
        {focusCards.map((card) => (
          <Link key={card.title} href={card.href} className="card-surface p-6 hover:-translate-y-1 transition-all duration-300 rounded-[2rem] border border-bone-500/20 shadow-sm hover:shadow-xl group">
            ...
            <p className="text-body font-heading font-black text-ink-950 tracking-tight">{card.title}</p>
            <p className="mt-3 text-body-sm text-text-secondary leading-relaxed">{card.description}</p>
            <div className="mt-auto pt-4 flex items-center justify-between">
               <p className="text-[10px] font-black text-ink-300 uppercase tracking-widest">{card.meta}</p>
               <span className="inline-flex items-center gap-1 text-[10px] font-black text-brand-600 uppercase tracking-tight">
                {card.cta}
      ...
      <div className="card-surface p-8 lg:p-10 border-none bg-ink-950 text-white relative overflow-hidden rounded-[2.5rem] shadow-2xl">
        ...
        <div className="flex flex-col md:flex-row items-start justify-between gap-8 relative z-10">
          ...
          <Link
            href={domino.href}
            className="flex-shrink-0 inline-flex items-center gap-3 px-8 py-4 rounded-2xl bg-gold-500 text-ink-950 font-black text-sm uppercase tracking-tight hover:bg-white hover:scale-105 transition-all shadow-lg active:scale-95"
          >
            {domino.cta}
        ...
        <div className="card-surface p-8 group hover:-translate-y-1 transition-all rounded-[2.5rem] bg-white border border-bone-500/20">
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              ...
              <p className="text-sm font-black text-ink-950 tracking-tight mt-1">Consistência da Rota</p>
            </div>
          </div>
        ...
        <div className="card-surface p-8 group hover:-translate-y-1 transition-all rounded-[2.5rem] bg-white border border-bone-500/20">
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              ...
              <p className="text-sm font-black text-ink-950 tracking-tight mt-1">Custo de Retenção</p>
            </div>
          </div>
          <div className="flex items-baseline gap-1">
            ...
            <span className="text-xs font-black text-ink-200 uppercase mb-1">/dia</span>
          </div>
          <p className="text-[11px] text-clay-500 font-black uppercase tracking-widest mt-6">
        ...
        <div className="card-surface p-8 group hover:-translate-y-1 transition-all rounded-[2.5rem] bg-white border border-bone-500/20">
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              ...
              <p className="text-sm font-black text-ink-950 tracking-tight mt-1">Sincronia de Entrega</p>
            </div>
          </div>
          <div className="flex items-baseline justify-between mb-2">
            ...
            <div className="bg-amber-100 px-3 py-1 rounded-full text-[10px] font-black text-amber-900 border border-amber-200">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
        ...
        <div className="lg:col-span-3 card-surface p-8 rounded-[3rem]">
          <div className="flex items-center justify-between mb-8">
            <h3 className="font-heading text-xl font-black text-ink-950 uppercase tracking-tight">Fluxo Operacional</h3>
            <Link href="/sprints" className="text-[10px] font-black text-brand-600 hover:text-ink-950 uppercase tracking-widest transition-colors flex items-center gap-2">
            ...
            {timeline.length > 0 ? timeline.map((item, i) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-base font-black text-ink-950 tracking-tight truncate">{item.label}</p>
                  <p className={`text-[11px] font-black uppercase tracking-wider mt-1 ${item.status === "urgent" ? "text-red-500" : "text-ink-300"}`}>
              ...
              <div className="text-center py-12 bg-sage-50/30 rounded-[2rem] border border-dashed border-sage-500/30">
                ...
                <p className="text-lg font-black text-ink-950">Frequência Limpa</p>
      ...
      <div className="space-y-6">
          <h3 className="font-heading text-xl font-black text-ink-950 uppercase tracking-tight">Atalhos Estratégicos</h3>
          ...
          {[
            ...
            >
              ...
              <p className="text-xs font-black text-ink-950 uppercase tracking-tight">
                {action.label}
              </p>
            </Link>
          ))}
    ...
    >
      <div className="card-surface p-8 rounded-[3rem] bg-white border border-bone-500/20 shadow-glass-sm hover:shadow-2xl transition-all duration-500 hover:-translate-y-1 relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-8 relative z-10">
          ...
          <div className="flex-1 max-w-md w-full grid grid-cols-2 gap-8">
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-end">
                <span className="text-[10px] font-black uppercase tracking-widest text-ink-300 leading-none">Energia</span>
                <span className="text-sm font-black text-ink-950 leading-none">{Math.round(aura.energy)}%</span>
              </div>
            ...
            {/* Sync Progress */}
             <div className="space-y-3">
              <div className="flex justify-between items-end">
                <span className="text-[10px] font-black uppercase tracking-widest text-ink-300 leading-none">Sincronia</span>
                <span className="text-sm font-black text-ink-950 leading-none">{Math.round((aura.experiencePoints / aura.xpToNextLevel) * 100)}%</span>
              </div>
              <div className="h-1.5 bg-ink-950/5 rounded-full overflow-hidden p-0.5 border border-bone-400/10">
                <motion.div 
                  initial={{ width: 0 }}
          ...
          <div className="flex items-center gap-6">
            {needsCare && (
              <div className="flex items-center gap-2 px-4 py-2 rounded-2xl bg-amber-50 border border-amber-200 text-[10px] font-black text-amber-900 uppercase tracking-tight animate-pulse">
