...
import { useMarketplaceStore, type BookingStatus, type EscrowStatus } from "@/stores/canonicalMarketplaceProviderStore";
...
const STATUS_MAP: Record<BookingStatus, { label: string; color: string; icon: typeof CheckCircle }> = {
  ...
  confirmed: { label: "Confirmada", color: "text-brand-500", icon: CheckCircle },
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="grid md:grid-cols-3 gap-4">
        ...
        <div className="card-surface p-4 text-center">
          <p className="font-heading font-bold text-h3 text-brand-500">R$ {summary.protectedValue}</p>
      ...
      <div className="flex gap-2">
        {(["all", "active", "completed"] as FilterTab[]).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`px-4 py-2 rounded-lg text-body-sm font-medium transition-colors ${tab === t ? "bg-brand-500 text-white" : "bg-cream-100 text-text-secondary hover:bg-cream-200"}`}
          >
                ...
                <div className="flex flex-col md:flex-row md:items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap gap-3 mt-1 text-caption text-text-muted">
                      ...
                      <span className="flex items-center gap-1"><Shield className="w-3 h-3 text-brand-500" />Escrow: {ESCROW_LABELS[booking.escrow]}</span>
                    </div>
                  </div>
