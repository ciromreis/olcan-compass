...
import { Companion, CompanionType, EvolutionStage, Ability, CompanionRoute, FearCluster } from '@olcan/ui-components'
    ...
    persist(
      (set, get) => ({
        ...
        fetchCompanions: async () => {
          ...
          try {
            const backendCompanions = await apiClient.getCompanions()
        ...
        createCompanion: async (archetypeId, name) => {
          ...
          set({ isLoading: true, error: null })
          try {
            const backendCompanion = await apiClient.createCompanion({
              name,
        ...
        performCareActivity: async (activity) => {
          ...
          try {
            ...
            if (activity.type === 'feed') {
              result = await apiClient.feedCompanion(companionId)
            } else if (activity.type === 'train') {
              result = await apiClient.trainCompanion(companionId)
            } else if (activity.type === 'play') {
              result = await apiClient.playWithCompanion(companionId)
            } else if (activity.type === 'rest') {
              result = await apiClient.restCompanion(companionId)
            } else {
