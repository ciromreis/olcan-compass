...
import { CATEGORY_LABELS, useMarketplaceStore } from "@/stores/canonicalMarketplaceProviderStore";
          ...
          return (
            <div key={msg.id} className={`flex gap-3 ${isUser ? "justify-end" : ""}`}>
              ...
              <div className={`max-w-[75%] p-4 rounded-xl ${isUser ? "bg-brand-500 text-white" : "card-surface"}`}>
                ...
                <p className={`text-caption mt-1 ${isUser ? "text-brand-200" : "text-text-muted"}`}>{new Date(msg.timestamp).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}</p>
              </div>
              {isUser && (
