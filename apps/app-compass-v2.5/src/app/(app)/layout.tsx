...
import {
  initGamificationBridge,
  initCompanionEvolutionBridge,
} from "@/lib/event-bus";
...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const bootstrap = async () => {
      ...
      if (hasSession && !user) {
        try {
          const success = await fetchProfile(false);
          if (!success) logout();
        } catch {
          logout();
        }
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <div
        className={`mx-auto flex min-h-screen max-w-[1700px] gap-4 px-3 py-3 lg:px-5 lg:py-5 transition-all duration-300`}
      >
        <aside
        ...
        >
          <div className="flex h-full flex-col">
            ...
            <div
              className={`border-b border-slate-100 pb-4 ${sidebarCollapsed ? "px-0 flex justify-center" : "px-2"}`}
            >
              {sidebarCollapsed ? (
                <button
                  type="button"
            ...
            <nav
              className={`flex-1 overflow-y-auto py-4 ${sidebarCollapsed ? "space-y-1 px-0" : "space-y-6 px-1"}`}
            >
              {sidebarCollapsed ? (
                // Icon-only mode
                <>
            ...
            <div
              className={`border-t border-slate-100 pt-3 ${sidebarCollapsed ? "flex justify-center" : ""}`}
            >
              {sidebarCollapsed ? (
                <button
                  type="button"
        ...
        <div className="min-w-0 flex-1">
          <div
            className={`grid gap-4 ${auraRailOpen ? "lg:grid-cols-[minmax(0,1fr)_22rem] xl:grid-cols-[minmax(0,1fr)_24rem]" : ""}`}
          >
          ...
          {MOBILE_PRIMARY_ITEMS.map((item) => {
            ...
            return (
              ...
              >
                ...
                />
                <span
                  className={`text-[10px] uppercase tracking-[0.18em] ${active ? "text-white" : "text-slate-500"}`}
                >
                  {item.label}
                </span>
                {active ? (
        ...
        >
          ...
          {submenu.item.children!.map((child) => {
            ...
            return (
              ...
              >
                <span
                  className={`h-1.5 w-1.5 rounded-full shrink-0 ${childActive ? "bg-white" : "bg-slate-300"}`}
                />
                {child.label}
              </Link>
            );
