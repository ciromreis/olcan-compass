...
const STATUS_MAP: Record<string, { label: string; color: string; icon: typeof CheckCircle }> = {
  approved: { label: "Aprovado", color: "text-brand-500", icon: CheckCircle },
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex gap-3">
        <div className="relative flex-1">
          ...
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar provedores..." aria-label="Buscar provedores" className="w-full pl-10 pr-4 py-2.5 rounded-lg border border-cream-500 bg-white text-text-primary placeholder:text-text-muted focus:outline-none focus:ring-2 focus:ring-brand-400 focus:border-transparent" />
        </div>
        <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="px-4 py-2.5 rounded-lg border border-cream-500 bg-white text-text-secondary text-body-sm font-medium focus:outline-none focus:ring-2 focus:ring-brand-400">
              ...
              {filtered.map((p) => {
                ...
                return (
                  <tr key={p.id} className={`border-b border-cream-200 hover:bg-cream-50 transition-colors ${p.status === "pending" ? "bg-clay-50/30" : ""}`}>
                    ...
                    <td className="py-3 px-4 text-center">{p.pei ? <span className="font-bold text-brand-500">{p.pei}</span> : <span className="text-text-muted">—</span>}</td>
                      ...
                      {p.status === "pending" ? (
                        <div className="flex gap-1">
                          <button
                            ...
                            className="px-2 py-1 rounded bg-brand-500 text-white text-caption font-medium"
                          >
                            Aprovar
                          </button>
                          <button
