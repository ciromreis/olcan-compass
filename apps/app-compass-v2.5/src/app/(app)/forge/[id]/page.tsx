...
import {
  ...
  CheckCircle, AlertCircle, Loader2, BookmarkPlus, Zap, Mic, Lock, FileText
} from "lucide-react";
...
export default function ForgeEditorPage() {
  ...
  const tabs = [
    ...(doc.type === "cv" ? [
      { href: `/forge/${docId}/cv-builder`, icon: FileText, label: "Construtor de CV" },
      { href: `/forge/${docId}/ats-optimizer`, icon: Target, label: "Otimizador ATS" }
    ] : []),
    ...
    <div className="max-w-6xl mx-auto space-y-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            disabled
            title="Em breve: refinamento com IA"
            className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg border border-cream-300 bg-cream-100 text-text-muted font-medium text-body-sm cursor-not-allowed opacity-60 transition-colors"
          >
        ...
        <div className="grid lg:grid-cols-4 gap-4 w-full h-full">
          <div className="lg:col-span-3 flex flex-col h-full">
            ...
            <div className="flex items-center gap-1 px-4 py-2 border-b border-cream-300 text-text-muted flex-wrap">
              ...
              <span className="w-px h-5 bg-cream-400 mx-1" />
              <button onClick={handleUndo} title="Desfazer (Ctrl+Z)" className="p-1.5 rounded hover:bg-cream-200 transition-colors disabled:opacity-40" disabled={historyIndexRef.current <= 0}><Undo2 className="w-4 h-4" /></button>
              <button onClick={handleRedo} title="Refazer (Ctrl+Y)" className="p-1.5 rounded hover:bg-cream-200 transition-colors disabled:opacity-40" disabled={historyIndexRef.current >= historyRef.current.length - 1}><Redo2 className="w-4 h-4" /></button>
            ...
            <div className="card-surface p-4 border border-brand-200 bg-brand-50/50">
              <div className="flex items-center justify-between mb-3">
                ...
                {doc.constraints?.targetScholarship && (
                  <span className="px-2 py-0.5 rounded-full bg-brand-100 text-brand-700 text-caption font-bold uppercase">
                    {doc.constraints.targetScholarship}
                  </span>
                )}
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="p-3 rounded-xl bg-white/40 border border-[#001338]/5 flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-caption font-bold text-[#001338]/60 uppercase tracking-widest">Impulso da Aura</p>
