...
const CARE_ACTIVITIES: { type: CareActivityType; icon: React.ComponentType<{ className?: string }>; label: string; xpReward: number; energyCost: number; description: string }[] = [
...
export default function CompanionPage() {
  ...
  const handleCareActivity = async (activity: typeof CARE_ACTIVITIES[number]) => {
    try {
...
function ActivityButton({ activity, currentEnergy, onClick }: { activity: typeof CARE_ACTIVITIES[number]; currentEnergy: number; onClick: () => void }) {
  ...
  return (
    ...
    >
      ...
      `}>
        {React.createElement(activity.icon, { className: 'w-6 h-6' })}
      </div>
