    ...
    try {
      ...
      try {
        await marketplaceApi.applyAsProvider({
          ...
          specializations: [
            SPECIALIZATION_MAP[form.specialization] || form.specialization,
          ],
    ...
    <div className="card-surface p-8">
      <div className="text-center mb-8">
        ...
        <p className="text-body text-text-secondary">
          Ofereça seus serviços no marketplace verificado
        </p>
      </div>
      {displayError && (
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            className="w-full px-4 py-2.5 rounded-lg border border-cream-500 bg-white text-text-primary placeholder:text-text-muted focus:outline-none focus:ring-2 focus:ring-brand-400 focus:border-transparent"
            required
          />
        </div>
        <div>
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-lg bg-clay-500 text-white font-heading font-semibold hover:bg-clay-600 disabled:opacity-50 transition-colors"
        >
          {loading
            ? "Criando conta e enviando candidatura..."
      ...
      <div className="mt-6 text-center">
        <Link
          href="/register"
          className="text-body-sm text-text-secondary hover:text-brand-500 transition-colors"
        >
          ← Voltar para cadastro padrão
        </Link>
      </div>
    </div>
  );
