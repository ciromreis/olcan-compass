...
@router.post("/{sprint_id}/tasks/bulk", response_model=SprintTaskListResponse, status_code=status.HTTP_201_CREATED)
async def create_tasks_bulk(
    sprint_id: UUID,
    request: SprintTaskBulkCreate,
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db)
):
    """Bulk-create tasks for a sprint in a single DB transaction.

    Replaces N concurrent POST /tasks calls from the frontend,
    which previously exhausted Neon connections and hit Vercel 10s timeout.
    """
    # Verify ownership
    sprint_result = await db.execute(
        select(UserSprint).where(
    ...
    # Update sprint task counters atomically
    sprint.total_tasks += len(created_tasks)
    sprint.updated_at = datetime.now(timezone.utc)

    await db.commit()

    # Refresh all tasks to get DB-generated fields
    for task in created_tasks:
        await db.refresh(task)

    return SprintTaskListResponse(
        items=[SprintTaskResponse.model_validate(t) for t in created_tasks],
