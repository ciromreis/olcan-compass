#!/usr/bin/env python3
"""
Simple script to seed archetypes for companion system
"""
...
from app.models.archetype import Archetype, Ability, EvolutionRequirement

# Create database connection - use SQLite directly
engine = create_engine("sqlite:///./app.db", connect_args={"check_same_thread": False})
SessionLocal = sessionmaker(autocommit=False, autoflush=False, bind=engine)

# Simple archetypes data
ARCHETYPES_DATA = [
    {
        "name": "strategist",
...
def seed_archetypes():
    ...
    try:
        ...
        from app.models.archetype import Base
        Base.metadata.create_all(bind=engine)
        
        # Check if archetypes already exist
        existing_count = db.query(Archetype).count()
        if existing_count > 0:
            print(f"Archetypes table already has {existing_count} records. Skipping seed.")
            return
