...
class PsychProfileResponse(PsychProfileBase):
    ...
    created_at: datetime
    updated_at: datetime
    # OIOS Gamification fields (nullable until user completes diagnostic)
    dominant_archetype: Optional[str] = None
    primary_fear_cluster: Optional[str] = None
    evolution_stage: int = 1
    kinetic_energy_level: float = 0.0
...
class PsychProfileUpdate(BaseModel):
    ...
    # OIOS archetype — set after onboarding diagnostic completes
    dominant_archetype: Optional[str] = None
    primary_fear_cluster: Optional[str] = None
