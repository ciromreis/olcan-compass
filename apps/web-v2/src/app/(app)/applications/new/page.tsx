    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="space-y-3">
        ...
        <div className="grid sm:grid-cols-3 gap-3">
          {APP_TYPES.map((type) => (
            <button key={type.id} onClick={() => setSelected(type.id)} className={`card-surface p-4 flex flex-col items-center gap-2 text-center transition-all ${selected === type.id ? "ring-2 ring-moss-500 bg-moss-50/50" : "hover:-translate-y-0.5"}`}>
              <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${selected === type.id ? "bg-moss-100" : "bg-cream-200"}`}>
      ...
      {selected && (
        <div className="card-surface p-6 space-y-4">
          ...
          <div>
            ...
            <input type="date" value={deadline} onChange={(e) => setDeadline(e.target.value)} className="w-full px-4 py-2.5 rounded-lg border border-cream-500 bg-white text-text-primary focus:outline-none focus:ring-2 focus:ring-moss-400 focus:border-transparent" />
          </div>
          <div>
            <label className="block text-body-sm font-medium text-text-primary mb-1.5">País</label>
            <select value={country} onChange={(e) => setCountry(e.target.value)} className="w-full px-4 py-2.5 rounded-lg border border-cream-500 bg-white text-text-primary focus:outline-none focus:ring-2 focus:ring-moss-400">
