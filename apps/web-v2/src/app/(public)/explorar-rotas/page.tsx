  ...
  return (
    <section className="max-w-5xl mx-auto px-6 py-20">
      <div className="text-center mb-16">
        <p className="text-caption font-heading font-semibold tracking-widest uppercase text-moss-400 mb-3">Rotas de Mobilidade</p>
      ...
      <div className="grid gap-6">
        {ROUTES.map((route) => (
          <div key={route.type} className="card-surface p-6 flex flex-col md:flex-row md:items-center gap-5 group hover:-translate-y-0.5 transition-transform cursor-pointer">
            <div className={`w-14 h-14 rounded-xl flex items-center justify-center flex-shrink-0 ${route.color === "moss" ? "bg-moss-50" : route.color === "clay" ? "bg-clay-50" : "bg-sage-50"}`}>
              <route.icon className={`w-6 h-6 ${route.color === "moss" ? "text-moss-500" : route.color === "clay" ? "text-clay-500" : "text-sage-500"}`} />
            </div>
            ...
            <ArrowRight className="w-5 h-5 text-text-muted group-hover:text-moss-500 transition-colors flex-shrink-0" />
          </div>
        ))}
      </div>
      <div className="text-center mt-12">
        <a href="/register" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-moss-500 text-white font-heading font-semibold hover:bg-moss-600 transition-colors shadow-md">
