      ...
      <div className="grid md:grid-cols-2 gap-4 mt-2">
        {plans.map((plan) => {
          ...
          return (
            <div
              key={plan.id}
              className={`rounded-2xl border p-5 ${current ? "border-brand-500 bg-brand-50" : "border-cream-400 bg-cream-50"}`}
            >
              <div className="flex items-start justify-between gap-3 mb-4">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${current ? "bg-brand-100" : "bg-white"}`}>
                      {plan.id === "premium" ? <Crown className="w-4 h-4 text-clay-500" /> : <Sparkles className="w-4 h-4 text-brand-500" />}
                    </div>
                ...
                {current && <span className="text-caption px-2 py-1 rounded-full bg-brand-500 text-white font-medium">Atual</span>}
              </div>
              ...
              <div className="space-y-2 mb-5">
                {plan.highlights.map((highlight) => (
                  <div key={highlight} className="flex items-start gap-2 text-body-sm text-text-secondary">
                    <Zap className="w-4 h-4 mt-0.5 text-brand-500 flex-shrink-0" />
                    <span>{highlight}</span>
                  </div>
                ))}
              </div>
