...
### 🔴 CRÍTICO: Export de Dossier é STUB (2026-04-22)

**Problema encontrado**: O `export_service.py` (617 linhas) tem a arquitetura CORRETA implementada:
...
```python
# export_service.py:388-400 (PDF stub)
...
**Impacto**: O usuário baixa um arquivo com extensão `.pdf` mas que abre como texto puro.

**Solução planejada**: Implementar geração real via Playwright + Jinja2 (ver [[Modelo_Core_Routes_Sprints_Tasks]]).
