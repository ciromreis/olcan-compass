 ...
 */
export const NanoBananaImage: React.FC<NanoBananaImageProps> = ({
  archetype,
  stage,
  route = 'academic',
  className,
  ...
  return (
    <div className={cn("relative group shrink-0", className)} style={{ width: size, height: size }}>
      <motion.div 
        className="absolute inset-0 rounded-full bg-white/5 backdrop-blur-2xl border border-white/20 shadow-2xl overflow-hidden ring-1 ring-white/10"
      ...
      >
        <motion.img 
          src={placeholderUrl}
          alt={prompt}
          className="w-full h-full object-cover opacity-60 mix-blend-plus-lighter group-hover:scale-105 transition-transform duration-1000"
        />
        
        {/* Dynamic Glow Layer */}
        <div className="absolute inset-0 bg-gradient-to-tr from-[#001338]/20 via-transparent to-white/30 pointer-events-none" />
      ...
      {/* Pulsing Aura */}
      <motion.div 
        className="absolute -inset-2 -z-10 rounded-full opacity-40 blur-xl bg-[#001338]/5"
        animate={{
          opacity: [0.2, 0.4, 0.2],
          scale: [1, 1.05, 1],
        }}
