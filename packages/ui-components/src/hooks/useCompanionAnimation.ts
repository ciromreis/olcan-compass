...
export const useCompanionAnimation = ({ 
  companionType, 
  evolutionStage, 
  isActive = false 
}: UseCompanionAnimationProps) => {
  const [animationState, setAnimationState] = useState('idle')
  const [mood, setMood] = useState<'happy' | 'excited' | 'sleepy' | 'focused'>('happy')
  ...
  const getEvolutionAnimation = () => {
    ...
    return animations[evolutionStage as keyof typeof animations] || animations.egg
  }
  ...
  const getTypeAnimation = () => {
    ...
    return typeAnimations[companionType as keyof typeof typeAnimations] || typeAnimations.strategist
  }
  ...
  const getParticleCount = () => {
    ...
    return particleCounts[evolutionStage as keyof typeof particleCounts] || 0
  }
  ...
  const getGlowIntensity = () => {
    ...
    return glowIntensities[evolutionStage as keyof typeof glowIntensities] || 0.1
  }
